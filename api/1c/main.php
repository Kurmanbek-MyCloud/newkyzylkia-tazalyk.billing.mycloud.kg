...
$logger = new MyLogger('api/1c/pay.log');
...
try {
	...
	if (is_array($response)) {
		...
		if ($response['action'] == 'check') {
			$logger->log("Запрос " . $response['action'] . ' ' . $response['comment']);
			print json_encode(array('success' => true, 'comment' => $response['comment'], 'result' => 0));
		} elseif ($response['action'] == 'pay') {
			$logger->log($response['cf_payment_source'] . ' ' . $response['comment'] . " сумма " . $amount . " номер платежа " . $response['cf_txnid']);
