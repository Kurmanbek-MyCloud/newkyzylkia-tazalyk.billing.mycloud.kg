...
function generateContactLs($ws_entity) {
	global $adb;
	$contact_id = explode('x', $ws_entity->getId())[1];

	if (strlen($contact_id) >= 5) {
		$contact_id_padded = $contact_id;
	} else {
		// Определяем количество нулей, которые нужно добавить перед числом
		$num_zeros = 4 - strlen($contact_id);

		// Добавляем единицу и нужное количество нулей перед числом
		$contact_id_padded = '1' . str_repeat('0', $num_zeros) . $contact_id;
	}

	$adb->pquery("UPDATE vtiger_contactdetails
                  SET cf_personal_contact_number = '$contact_id_padded'
                  WHERE contactid = ?", array($contact_id));
}
?>