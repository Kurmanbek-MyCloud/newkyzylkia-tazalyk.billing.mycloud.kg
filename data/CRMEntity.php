...
class CRMEntity {
	...
	function insertIntoCrmEntity($module, $fileid = '') {
		...
		switch ($module) {
			case 'HelpDesk':
				$entityFieldNames = array('ticket_title');
				break;
			case 'Documents':
				$entityFieldNames = array('notes_title');
				break;
		}
...
//Added by Richie
	...
	function trash($module, $id) {
		...
		if ($module == 'Invoice' || $module == 'Payments' || $module == 'Penalty') {
			$this->updateDebt($module, $id);
		}
	}

	function updateDebt($module, $id) {
		global $adb;
		if ($module == 'Invoice') {
			$object_id = $adb->run_query_field("SELECT cf_estate_id FROM vtiger_invoice WHERE invoiceid = $id");

			$penaltyFocus = CRMEntity::getInstance('Penalty');
			$penaltyResult = $adb->pquery("SELECT penaltyid FROM vtiger_penalty WHERE cf_to_ivoice = ?", array($id));
			$numRows = $adb->num_rows($penaltyResult);
			for ($i = 0; $i < $numRows; $i++) {
				$penaltyId = $adb->query_result($penaltyResult, $i, 'penaltyid');
				$penaltyFocus->mark_deleted($penaltyId);
			}
		} elseif ($module == 'Payments') {
			$object_id = $adb->run_query_field("SELECT cf_paid_object FROM vtiger_payments WHERE paymentsid = $id");

		} elseif ($module == 'Penalty') {
			$object_id = $adb->run_query_field("SELECT cf_estate_id FROM vtiger_invoice inv
												INNER JOIN vtiger_penalty vp ON inv.invoiceid = vp.cf_to_ivoice 
												WHERE vp.penaltyid = $id");
		}

		$setype = '';
		// Определяем setype
		if (in_array($module, ['Payments', 'Invoice', 'Penalty'])) {
		...
		foreach ($with_crmid as $relcrmid) {
			...
			if ($with_module == 'Documents') {
				...
				$checkpresence = $adb->pquery("SELECT crmid FROM vtiger_crmentityrel WHERE
					(crmid = ? AND module = ? AND relcrmid = ? AND relmodule = ?) OR (relcrmid=? AND relmodule=? AND crmid=? AND module=?)", array($crmid, $module, $relcrmid, $with_module, $crmid, $module, $relcrmid, $with_module));
				// Relation already exists? No need to add again
				if ($checkpresence && $adb->num_rows($checkpresence)) {
					continue;
				} elseif ($with_module == 'Estates' && $module == 'Services') {
					$adb->pquery("INSERT INTO vtiger_crmentityrel(crmid, module, relcrmid, relmodule) VALUES(?,?,?,?)", array($relcrmid, $with_module, $crmid, $module));
				} else {
