...
function getAssociatedProducts($module, $focus, $seid = '', $refModuleName = false) {
	...
	for ($i = 1; $i <= $num_rows; $i++) {
		...
		if ($module == 'Invoice') {
			...
			// база для начисления (номер счётчика)
			$accrualBase = $adb->query_result($result, $i - 1, 'accrual_base');
			$product_Detail[$i]['accrualBase' . $i] = $accrualBase;

			// ID счётчика по номеру
			if (!empty($accrualBase)) {
				$meterRes = $adb->pquery("SELECT metersid FROM vtiger_meters WHERE meter_number = ? LIMIT 1", array($accrualBase));
				if ($adb->num_rows($meterRes) > 0) {
					$product_Detail[$i]['metersId' . $i] = $adb->query_result($meterRes, 0, 'metersid');
				}
			}

			// предыдущее показание
			$previousReading = $adb->query_result($result, $i - 1, 'previous_reading');
			$product_Detail[$i]['previousReading' . $i] = $previousReading;

			// текущее показание
			$currentReading = $adb->query_result($result, $i - 1, 'current_reading');
			$product_Detail[$i]['currentReading' . $i] = $currentReading;

			// ID показаний из модуля Readings
			$prevReadingId = $adb->query_result($result, $i - 1, 'prev_reading_id');
			$product_Detail[$i]['prevReadingId' . $i] = $prevReadingId;
			$curReadingId = $adb->query_result($result, $i - 1, 'cur_reading_id');
			$product_Detail[$i]['curReadingId' . $i] = $curReadingId;

			// Подгрузка значений показаний для отображения
			if (!empty($prevReadingId)) {
				$prevRes = $adb->pquery("SELECT meter_reading, cf_reading_date FROM vtiger_readings WHERE readingsid = ?", array($prevReadingId));
				if ($adb->num_rows($prevRes) > 0) {
					$product_Detail[$i]['prevReadingDisplay' . $i] = $adb->query_result($prevRes, 0, 'meter_reading') . ' (' . $adb->query_result($prevRes, 0, 'cf_reading_date') . ')';
				}
			}
			if (!empty($curReadingId)) {
				$curRes = $adb->pquery("SELECT meter_reading, cf_reading_date FROM vtiger_readings WHERE readingsid = ?", array($curReadingId));
				if ($adb->num_rows($curRes) > 0) {
					$product_Detail[$i]['curReadingDisplay' . $i] = $adb->query_result($curRes, 0, 'meter_reading') . ' (' . $adb->query_result($curRes, 0, 'cf_reading_date') . ')';
				}
			}
		}
