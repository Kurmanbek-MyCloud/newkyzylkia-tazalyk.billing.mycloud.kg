...
function saveInventoryProductDetails(&$focus, $module, $update_prod_stock = 'false', $updateDemand = '') {
	...
	$prod_seq = 1;
	$deductTotalAmount = vtlib_purify($_REQUEST['deduct_taxes_total_amount']);
	$shippingHandlingCharge = vtlib_purify($_REQUEST['shipping_handling_charge']);
	$preTaxTotal = vtlib_purify($_REQUEST['pre_tax_total']);
	$grandTotal = vtlib_purify($_REQUEST['grand_total']);
	$taxFinal = vtlib_purify($_REQUEST['tax_final']);
	$chargeTaxTotal = vtlib_purify($_REQUEST['s_h_percent']);
	$netTotal = vtlib_purify($_REQUEST['nettotal']);
	$monthCount = (int) vtlib_purify($_POST['monthCount']); // месяцы
	$tax1 = (int) vtlib_purify($_POST['tax1']);
	$tax2 = (int) vtlib_purify($_POST['tax2']);
	$tax3 = (int) vtlib_purify($_POST['tax3']);
	// var_dump($tax1);
	// exit();
	...
	for ($i = 1; $i <= $tot_no_prod; $i++) {
		...
		$productTotal = vtlib_purify($_REQUEST['producttotal' . $i]);
		$netPrice = vtlib_purify($_REQUEST['netprice' . $i]);
		...
		if ($module == 'Invoice') {
			...
			$prevReadingId = vtlib_purify($_REQUEST['prevReadingId' . $i]);
			if (empty($prevReadingId)) $prevReadingId = null;
			$curReadingId = vtlib_purify($_REQUEST['curReadingId' . $i]);
			if (empty($curReadingId)) $curReadingId = null;
		}
		...
		if ($module == 'Invoice') {// добавил месяцы
			$query = 'INSERT INTO vtiger_inventoryproductrel
						...
						accrual_base, previous_reading, current_reading, month_count, tax1, tax2, tax3, prev_reading_id, cur_reading_id)
						VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
			...
			$qparams = array($focus->id, $prod_id, $prod_seq, $qty, $listprice, $comment, $description, $purchaseCost,
				...
				$accrualBase, $previousReading, $currentReading, $monthCount, $tax1, $tax2, $tax3, $prevReadingId, $curReadingId);
		...
		// Отметить показания как использованные в счёте
		if ($module == 'Invoice') {
			if (!empty($prevReadingId)) {
				$adb->pquery("UPDATE vtiger_readings SET cf_used_in_bill = 1 WHERE readingsid = ?", array($prevReadingId));
			}
			if (!empty($curReadingId)) {
				$adb->pquery("UPDATE vtiger_readings SET cf_used_in_bill = 1 WHERE readingsid = ?", array($curReadingId));
			}
		}
