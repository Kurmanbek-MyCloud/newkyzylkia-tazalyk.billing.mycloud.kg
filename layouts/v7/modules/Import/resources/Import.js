...
if (typeof (Vtiger_Import_Js) == 'undefined') {
    ...
    Vtiger_Import_Js = {
                ...
                app.request.post({data: formData}).then(function(err, response) {
                    ...
                    if(!err){
                        if (jQuery('#scheduleImportStatus').length > 0) {
                            ...
                            app.helper.showSuccessNotification({message: app.vtranslate('Import Completed.')});
        ...
        checkInvoiceFile: function(e) {
            var filePath = jQuery('#invoice_file').val();
            if (filePath != '') {
                var fileExtension = filePath.split('.').pop();
                if (fileExtension == 'xlsx' || fileExtension == 'xls'){
                    ...
                    app.helper.showAlertNotification({message:"Неправильный формат загружаемого файла"})
                    jQuery('#importFileDetails').text('');
                }
        ...
        uploadInvoiceFile: function(){
            var form = $('#importInvoice')[0]; 
            var formData = new FormData(form);
            var file = $('input[name=invoice_file]').val()
            if (file == '') 
                app.helper.showAlertNotification({message:"Не выбран файл"})
                ...
                $.ajax({
                    ...
                    success: function(result) {
                        app.helper.hideProgress()
                        if (result.success) 
                            app.helper.showSuccessNotification({message:`Успешно загружено <br>
                                                                         Создано: ${result.created} <br> 
                                                                         Ошибок: ${result.errors} <br>
                                                                         Дубликатов: ${result.exist} <br>
                                                                         Не найденных абонентов: ${result.notFindedLS}`},{delay : 10000})
                        else 
                            app.helper.showErrorNotification({message:result.message})
                    },
                    error: function(err) {
                        app.helper.hideProgress()
                        app.helper.showErrorNotification({message:'Сервис недоступен'})
                    }
                });
            }
            else{
