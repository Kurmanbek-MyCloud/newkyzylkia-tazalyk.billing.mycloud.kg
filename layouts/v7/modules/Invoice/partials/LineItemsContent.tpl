...
{strip}
    ...
    {assign var="prevReadingId" value="prevReadingId"|cat:$row_no}
    {assign var="curReadingId" value="curReadingId"|cat:$row_no}
	...
	{if $PRODUCT_EDITABLE}
		<td>
			...
			{else}
				{if $COMMENT_EDITABLE}
					<div class="hidden"><br><textarea id="{$comment}" name="{$comment}" class="lineItemCommentBox">{decode_html($data.$comment)}</textarea></div>
				{/if}
			{/if}
		</td>
	{/if}
	...
	<td>
		<select id="meterSelect{$row_no}" class="meterSelect form-control" style="min-width:150px;">
			<option value="">-- Счётчик --</option>
		</select>
		<input type="hidden" id="{$accrualBase}" name="{$accrualBase}" value="{$data.$accrualBase}" class="accrualBaseHidden" />
	</td>

	<td>
		<select id="prevReadingSelect{$row_no}" class="readingSelect prevReadingSelect form-control" style="min-width:150px;">
			<option value="">-- Выберите --</option>
		</select>
		<input type="hidden" id="{$prevReadingId}" name="{$prevReadingId}" value="{$data.$prevReadingId}" class="prevReadingIdField" />
		<a href="javascript:void(0)" class="addReadingBtn btn btn-xs btn-success" style="margin-top:4px;" title="Добавить показание">
			<i class="fa fa-plus"></i> Добавить
		</a>
	</td>

	<td>
		<select id="curReadingSelect{$row_no}" class="readingSelect curReadingSelect form-control" style="min-width:150px;">
			<option value="">-- Выберите --</option>
		</select>
		<input type="hidden" id="{$curReadingId}" name="{$curReadingId}" value="{$data.$curReadingId}" class="curReadingIdField" />
		<a href="javascript:void(0)" class="addReadingBtn btn btn-xs btn-success" style="margin-top:4px;" title="Добавить показание">
			<i class="fa fa-plus"></i> Добавить
		</a>
	</td>
	...
	<td>
		<input id="{$qty}" name="{$qty}" type="text" class="qty smallInputBox inputElement"
			   data-rule-required=true data-rule-positive=true  value="{if !empty($data.$qty)}{$data.$qty}{else}0{/if}"
	...
	{if $LIST_PRICE_EDITABLE}
		<td>
			<div>
				{* <input type="hidden" class="old_price" value="{$data.$listPrice}"> *}
			...
			{$ITEM_DISCOUNT_AMOUNT_EDITABLE = false}
			{$ITEM_DISCOUNT_PERCENT_EDITABLE = false}
