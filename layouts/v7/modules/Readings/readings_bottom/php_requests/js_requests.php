...
if ($input['action'] == 'getMetersInfo') {
  ...
  foreach ($meters_results as $row) {
    ...
    $meters[$metersid] = [
      ...
      'meter_number' => $row['meter_number'],
      'cf_meter_object_link' => $row['cf_meter_object_link'],
      'cf_number_digits' => $row['cf_number_digits'],
      'readings' => [],
    ];
...
ORDER BY vr.cf_reading_date DESC, vr.meter_reading DESC
LIMIT 5");
  ...
  foreach ($meters as &$meter) {
    $meter['readings'] = array_reverse($meter['readings']);
  }
  unset($meter); // Убираем ссылку на последний элемент
...
if ($input['action'] == 'updateReadings') {
  ...
  $readings->set('cf_used_in_bill', $usedInBill); // Изменение пользователя
...
if ($input['action'] == 'getReadingsByEstate') {
  $estatesid = intval($input['estatesid']);

  $results = $adb->run_query_allrecords("SELECT vr.readingsid, vr.meter_reading, vr.cf_reading_date,
      vr.cf_used_in_bill, vr.cf_reading_meter_link, vm.meter_number
      FROM vtiger_readings vr
      INNER JOIN vtiger_crmentity vc ON vr.readingsid = vc.crmid
      INNER JOIN vtiger_meters vm ON vr.cf_reading_meter_link = vm.metersid
      INNER JOIN vtiger_crmentity vc2 ON vm.metersid = vc2.crmid
      WHERE vc.deleted = 0
      AND vc2.deleted = 0
      AND vm.cf_deactivated = 0
      AND vm.cf_meter_object_link = $estatesid
      ORDER BY vm.meter_number ASC, vr.cf_reading_date DESC, vr.meter_reading DESC");
  ...
  foreach ($results as $row) {
    $readings[] = [
      ...
      'meter_reading' => $row['meter_reading'],
      'cf_reading_date' => $row['cf_reading_date'],
      'cf_used_in_bill' => $row['cf_used_in_bill'],
      'meter_number' => $row['meter_number'],
      'metersid' => $row['cf_reading_meter_link'],
    ];
  }
...
if ($input['action'] == 'getUserGroup') {
  $userid = intval($input['userid']);

  $result = $adb->pquery(
    "SELECT g.groupid, g.groupname
     FROM vtiger_users2group u2g
     INNER JOIN vtiger_groups g ON g.groupid = u2g.groupid
     WHERE u2g.userid = ?
     LIMIT 1",
    array($userid)
  );
  ...
  if ($adb->num_rows($result) > 0) {
    ...
    echo json_encode(['groupid' => null, 'groupname' => null]);
  }
  exit();
}
?>