...
Vtiger_Edit_Js("Inventory_Edit_Js", {
	...
	isGroupIncTaxMode: function () {
		return (this.taxTypeElement.val() == Inventory_Edit_Js.groupTaxIncType) ? true : false;
	},
	//SalesPlatform.ru end
	...
	updateRowNumberForRow: function (lineItemRow, expectedSequenceNumber, currentSequenceNumber) {
		...
		var idFields = new Array('productName', 'subproduct_ids', 'hdnProductId', 'purchaseCost', 'margin',
			...
			'accrualBase', 'previousReading', 'currentReading', 'tax1', 'tax2', 'tax3');
	...
	calculateLineItemTotal: function (lineItemRow) {
		...
		// Умножаем тариф × количество
		var lineItemTotal = parseFloat(listPrice) * parseFloat(quantity);
	...
	// 	var self = this;
	// 	var totalAfterDiscount = this.getTotalAfterDiscount(lineItemRow);
	// 	// Находим элемент, в котором находится таблица с налогами
	// 	var lineItemTable = jQuery('#lineItemResult');
	// 	// Находим проценты налогов внутри указанной таблицы
	// 	var taxPercentages = jQuery('.groupTaxPercentage', lineItemTable);
	// 	var taxTotal = 0;
	// 	jQuery.each(taxPercentages, function (index, domElement) {
	// 		var taxPercentage = jQuery(domElement);
	// 		var individualTaxRow = taxPercentage.closest('tr');
	...
	// 			individualTaxTotal = parseFloat(individualTaxTotal.toFixed(self.numOfCurrencyDecimals));
	// 		}
	// 		// Добавляем текущий налог к общей сумме
	// 		taxTotal += individualTaxTotal;
	// 		// Устанавливаем рассчитанный налог в соответствующую ячейку
	// 		individualTaxRow.find('.taxTotal').val(individualTaxTotal);
	// 		// Устанавливаем рассчитанный налог в модальное окно
	// 		var taxClass = '.taxTotal' + (index + 1);
	// 		jQuery(taxClass).val(individualTaxTotal);
	// 	});
	// 	// Преобразуем итоговый налог в строку с фиксированным количеством знаков после запятой
	// 	taxTotal = taxTotal.toFixed(self.numOfCurrencyDecimals);
	// 	// Устанавливаем итоговый налог
	// 	this.setLineItemTaxTotal(lineItemRow, taxTotal);
	// },
