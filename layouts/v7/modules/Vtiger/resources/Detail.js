...
Vtiger.Class("Vtiger_Detail_Js", {
	...
	registerEventsForRelatedList: function () {
		...
		detailContentsHolder.on('click', 'a.relationDelete', function (e) {
			...
			// console.log(relatedController.parentRecordId);
						...
						function (e) {
							...
							relatedController.deleteRelation([relatedRecordid]).then(function (response) {
								...
								var params = {
									...
									'action': 'RelationAjax',
									'mode': 'updateWeight',
									'src_record': relatedController.parentRecordId,
		...
		detailContentsHolder.on('click', 'a.relationTrash', function (e) {
			e.stopImmediatePropagation();
			var element = jQuery(e.currentTarget);
			var row = element.closest('tr');
			var relatedRecordid = row.data('id');
	...
	*/
	...
	registerAjaxEditSaveEvent: function (contentHolder) {
		...
		contentHolder.on('click', '.inlineAjaxSave', function (e) {
			...
			if (previousValue == ajaxEditNewValue) {
				...
				var labelElement = $("td#Estates_detailView_fieldLabel_cf_apartment_number");
				var valueElement = $("td#Estates_detailView_fieldValue_cf_apartment_number");

				if (fieldValue == 'Офис') {
					labelElement.hide();
					valueElement.hide();
					$('[data-block="LBL_OFFICE_INFORMATION"]').removeClass('hide');
				} else if (fieldValue == 'Дом') {
					labelElement.hide();
					valueElement.hide();
					$('[data-block="LBL_OFFICE_INFORMATION"]').addClass('hide');
				} else if (fieldValue == 'Квартира') {
					$('[data-block="LBL_OFFICE_INFORMATION"]').addClass('hide');
					labelElement.show();
					valueElement.show();
					$('[data-block="LBL_OFFICE_INFORMATION"]').addClass('hide');
	...
	trackLinkClick: function (linkId) {
		console.log('here')
	},
