...
$(document).ready(function () {
    var typeSelect = $('select[name="cf_estate_type"]');
    var entityType = $('select[name="cf_object_type"]');

    function toggleFields() {
        var type = typeSelect.find('option:selected').val();
        var entity = entityType.find('option:selected').val();

        var apartmentLabelCell = $('#Estates_editView_fieldName_cf_apartment_number').closest('td.fieldValue').prev('td.fieldLabel');
        var apartmentValueCell = $('#Estates_editView_fieldName_cf_apartment_number').closest('td.fieldValue');
        var residentsLabelCell = $('#Estates_editView_fieldName_cf_number_of_residents').closest('td.fieldValue').prev('td.fieldLabel');
        var residentsValueCell = $('#Estates_editView_fieldName_cf_number_of_residents').closest('td.fieldValue');

        var estateTypeLabel = $('td.fieldLabel:contains("Тип недвижимости")');
        var estateTypeValue = estateTypeLabel.next('td.fieldValue');

        // Скрыть/показать блоки и поля
        if (entity === 'Физ. лицо') {
            estateTypeLabel.show();
            estateTypeValue.show();
            apartmentLabelCell.show();
            apartmentValueCell.show();
            residentsLabelCell.show();
            residentsValueCell.show();
            $('[data-block="LBL_LEGAL_ENTITY_INFORMATION"]').addClass('hide');
            $('[data-block="LBL_OFFICE_INFORMATION"]').addClass('hide');

        } else if (entity === 'Юр. лицо') {
            estateTypeLabel.hide();
            estateTypeValue.hide();
            apartmentLabelCell.hide();
            apartmentValueCell.hide();
            residentsLabelCell.hide();
            residentsValueCell.hide();
    ...
    // Обработчики изменений выбора
    typeSelect.change(toggleFields);
    entityType.change(toggleFields);
});
