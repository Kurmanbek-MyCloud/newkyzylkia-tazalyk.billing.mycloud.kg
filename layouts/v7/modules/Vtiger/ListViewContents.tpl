					...
					<th {if $COLUMN_NAME eq $LISTVIEW_HEADER->get('name')} nowrap="nowrap" {/if}>
						<a href="#" class="{if $NO_SORTING}noSorting{else}listViewContentHeaderValues{/if}" {if !$NO_SORTING}data-nextsortorderval="{if $COLUMN_NAME eq $LISTVIEW_HEADER->get('name')}{$NEXT_SORT_ORDER}{else}ASC{/if}" data-columnname="{$LISTVIEW_HEADER->get('name')}"{/if} data-field-id='{$LISTVIEW_HEADER->getId()}'>
							{if !$NO_SORTING}
								...
								{/if}
							{/if}
							{if {vtranslate($LISTVIEW_HEADER->get('label'), $LISTVIEW_HEADER->getModuleName())} eq 'Населенный пункт'}&nbsp;Населенный&nbsp;<br>
									&nbsp;пункт&nbsp;
								{elseif {vtranslate($LISTVIEW_HEADER->get('label'), $LISTVIEW_HEADER->getModuleName())} eq 'Сумма всех платежей'}&nbsp;Сумма всех&nbsp;<br>
								&nbsp;платежей&nbsp;
								{elseif {vtranslate($LISTVIEW_HEADER->get('label'), $LISTVIEW_HEADER->getModuleName())} eq 'Сумма всех счетов'}&nbsp;Сумма всех&nbsp;<br>
								&nbsp;счетов&nbsp;
								{else}
				...
				<tbody class="overflow-y">
					...
					{foreach item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS}
						...
						<span class="fieldValue">
								<span class="value" {if (is_numeric($LISTVIEW_ENTRY_VALUE) and $LISTVIEW_ENTRY_VALUE > 0 and $LISTVIEW_HEADER->get('name') == 'cf_balance')} style="color:#ca2913" {elseif (is_numeric($LISTVIEW_ENTRY_VALUE) and $LISTVIEW_ENTRY_VALUE < 0 and $LISTVIEW_HEADER->get('name') == 'cf_balance')} style="color:#489e56" {else} style="color:#000" {/if}>
