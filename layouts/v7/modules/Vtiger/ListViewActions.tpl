    ...
    <div id="listview-actions" class="listview-actions-container">
        ...
        <div class = "row">
            ...
            <div class="btn-group listViewActionsContainer" role="group" aria-label="...">
                    ...
                    <div class="btn-group listViewMassActions" role="group">
                        ...
                        <ul class="dropdown-menu" role="menu">
                            ...
                            {if $MODULE eq 'Invoice'}
                                 {assign var=USER_ROLE value=$CURRENT_USER_MODEL->getRole()}
                                {if $USER_ROLE eq 'H7' or $USER_ROLE eq 'H2'}
                                    <li class="hide">
                                        <a id="{$MODULE}_listView_massAction_LBL_PRINT_PDF" target="_blank" onclick="Vtiger_List_Js.triggerPrintPDF_for_electrocity()">
                                            Распечатать счет PDF
                                        </a>
                                    </li>
                                    <li class="divider hide"></li>
                                {/if}
                ...
                <div class='col-md-6'>
               {* {assign var=extraData value=$LISTVIEW_ENTRIES.extraData} *}
{* <p>Total: {$extraData.total}</p> *}
                ...
                {if $LISTVIEW_ENTRIES_COUNT eq '0' and $REQUEST_INSTANCE and $REQUEST_INSTANCE->isAjax()}
                    ...
                    {if $VIEWID}
                        ...
                        {/if}
                        {if $MODULE == 'Invoice'}
                        <!-- <span style = "color: red;"><b>Добавить информацию как генерировать счета</b> </span><br> -->
                        {* <span style = "color: red;"><b>Просим предоставить все показания счетчиков до указанной даты.</b> </span> *}
                        {/if}


            {if $MODULE eq 'Readings'}
                <button type="button" class="btn btn-light" id="fetchQuant">
                    Общ кол. показаний
                </button>
                <script>
                    (function () {
                        var button = document.getElementById('fetchQuant');
                        if (button) {
                            button.onclick = function () {
                                window.open('/total_readings/main.php', '_blank');
                            };
                        }
                    })();
                </script>
            ...
            {if $MODULE eq 'Invoice'}
                <button type="button" class="btn btn-light" id="monitoring_invoice">
                    Мониторинг счетов
                </button>
                <script>
                    (function () {
                        var button = document.getElementById('monitoring_invoice');
                        if (button) {
                            button.onclick = function () {
                                window.open('/redis-invoices/page_status/tabs_dashboard.php', '_blank');
                            };
                        }
                    })();
                </script>
            ...
            {if $MODULE eq 'Invoice'}
                <button type="button" class="btn btn-light" id="generate_invoice">
                    Генерация счетов
                </button>
                <script>
                    (function () {
                        ...
                        if (button) {
                            button.onclick = function () {
                                window.open('/redis-invoices/invoiceModal.php?source=bulk', '_blank');
                            };
                        }
                    })();
                </script>
