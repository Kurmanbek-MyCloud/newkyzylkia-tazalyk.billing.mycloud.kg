...
{strip}
	...
	<div class="relatedContainer">
				...
				<table id="listview-table" class="table listview-table">
					<thead>
						<tr class="listViewHeaders">
							{if $RELATED_MODULE_NAME eq 'Invoice' && $MODULE eq 'Estates'}
								<th style="width:30px;text-align:center;">
									<input type="checkbox" id="relatedInvoiceSelectAll" title="Выбрать все">
								</th>
							{/if}
					...
					{foreach item=RELATED_RECORD from=$RELATED_RECORDS}
						<tr class="listViewEntries" data-id='{$RELATED_RECORD->getId()}' 
							...
							<td style="width:30px;text-align:center;">
								<input type="checkbox" class="relatedInvoiceCheckbox" value="{$RELATED_RECORD->getId()}" onclick="event.stopPropagation();">
							</td>
						{/if}
						<td class="related-list-actions">
								<span class="actionImages">&nbsp;&nbsp;&nbsp;
									...
									{if $IS_DELETABLE}
										{if $RELATED_MODULE_NAME eq 'Services' or $RELATED_MODULE_NAME eq 'Products'}
										<a class="relationDelete"><i title="{vtranslate('LBL_UNLINK', $MODULE)}" class="vicon-linkopen"></i></a>
										{else}
										<a class="relationTrash"><i title="{vtranslate('LBL_DELETE', $MODULE)}" class="fa fa-trash"></i></a>
										{/if}
									{/if}
								</span>
		...
		<script type="text/javascript">
			(function() {
				// Берём контейнер динамически — vtiger перезагружает related list через AJAX
				function getContainer() {
					return jQuery('#listview-table').closest('.relatedContainer');
				}

				// Select All checkbox
				jQuery(document).on('change', '#relatedInvoiceSelectAll', function() {
					var isChecked = jQuery(this).is(':checked');
					getContainer().find('.relatedInvoiceCheckbox').prop('checked', isChecked);
					updatePrintPdfButton();
				});

				// Individual checkbox change
				jQuery(document).on('change', '.relatedInvoiceCheckbox', function() {
					var $c = getContainer();
					var total = $c.find('.relatedInvoiceCheckbox').length;
					var checked = $c.find('.relatedInvoiceCheckbox:checked').length;
					$c.find('#relatedInvoiceSelectAll').prop('checked', total === checked);
					updatePrintPdfButton();
				});

				// Update button state
				function updatePrintPdfButton() {
					var checked = getContainer().find('.relatedInvoiceCheckbox:checked').length;
					jQuery('#printInvoicePdfBtn').prop('disabled', checked === 0);
				}

				// Print PDF button click
				jQuery(document).on('click', '#printInvoicePdfBtn', function() {
					var selectedIds = [];
					getContainer().find('.relatedInvoiceCheckbox:checked').each(function() {
					...
					var url = '/PrintPDF/PrintPDF_for_invoice.php?module=Invoice'
						+ '&selectedIds=' + encodeURIComponent(JSON.stringify(selectedIds))
						+ '&viewname=&params=&_t=' + Date.now();
