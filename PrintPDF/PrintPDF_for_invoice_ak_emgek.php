...
require_once 'payment_services/QRApiRSK.php';
...
function getHtml($invoiceId, $qrCodeData, $allInvoicesData)
{
    ...
    $html2 = "	
			...
			<tr class=\"table-head\">
				<td  width=\" 45% \" colspan=\"4\">Коммуналдык кызмат / Ком. услуга</td>
				<td width=\" 20% \" style=\"text-align: center;\">Количество</td>
				<td width=\" 20% \" style=\"text-align: center;\">Тариф</td>	
    ...
    foreach ($services as $row) {
        ...
        $html2 .= "	
			...
			<tr>
				<td width=\" 45% \" colspan=\"4\">$serviceName</td>
				<td width=\" 20% \"style=\"text-align: center;\">$quantity</td>
				<td width=\" 20% \"style=\"text-align: center;\">$listPrice</td>
				<td width=\" 15% \"style=\"text-align: right;\">$margin</td>
				</tr>";
        $temp_summ += $margin;
    }

    // Генерация QR-кода если есть задолженность и настройки QR
    $qrCodeImageBase64 = '';
    if (!empty($qrCodeData)) {
    ...
    if ($previous_debt > 0) {
    	...
    	// Генерация QR-кода для оплаты через РСК Банк
	// QR встраивается в HTML как <img> — TCPDF сам позиционирует при page break
	$qrImageHtml = '';
	$qrTempFile = null;
	$qrLogFile = __DIR__ . '/../logs/qr_debug.log';
	file_put_contents($qrLogFile, date('Y-m-d H:i:s') . " [DEBUG] Account: $personalAccount, Amount: $debt\n", FILE_APPEND);
    
	if ($debt > 0) {
        try {
            $qrApi = new QRApiRSK();
			$qrResult = $qrApi->generateQR($personalAccount, $debt, 2);
			$qrImage = $qrResult['response']['qrImage'] ?? '';
			file_put_contents($qrLogFile, date('Y-m-d H:i:s') . " [DEBUG] QR Response received, qrImage starts with: " . substr($qrImage, 0, 50) . "\n", FILE_APPEND);
			if (!empty($qrImage)) {
				// Извлекаем чистый base64 из data URI
				$base64Data = $qrImage;
				if (strpos($qrImage, 'base64,') !== false) {
					$base64Data = explode('base64,', $qrImage)[1];
				}
				$qrPngData = base64_decode($base64Data);
				if ($qrPngData !== false) {
					file_put_contents($qrLogFile, date('Y-m-d H:i:s') . " [DEBUG] QR decoded, size: " . strlen($qrPngData) . " bytes\n", FILE_APPEND);

					// Конвертируем PNG→JPEG (убираем прозрачность, которую TCPDF не рендерит)
					$pngImage = imagecreatefromstring($qrPngData);
					if ($pngImage) {
						$w = imagesx($pngImage);
						$h = imagesy($pngImage);
						$jpgImage = imagecreatetruecolor($w, $h);
						$white = imagecolorallocate($jpgImage, 255, 255, 255);
						imagefill($jpgImage, 0, 0, $white);
						imagecopy($jpgImage, $pngImage, 0, 0, 0, 0, $w, $h);
						$qrTempFile = sys_get_temp_dir() . '/qr_' . uniqid() . '.jpg';
                        imagedestroy($pngImage);

                        // Сохраняем JPEG прямо в буфер памяти, не в файл
                        ob_start();
                        imagejpeg($jpgImage, null, 100);
                        $jpegData = ob_get_clean();
                        imagedestroy($jpgImage);

                        // Встраиваем base64 прямо в src — TCPDF это поддерживает
                        $jpegBase64 = base64_encode($jpegData);
                        $qrImageHtml = '<img src="data:image/jpeg;base64,' . $jpegBase64 . '" width="100" height="100">';
						file_put_contents($qrLogFile, date('Y-m-d H:i:s') . " [DEBUG] QR JPEG saved: $qrTempFile\n", FILE_APPEND);
					}
				}
			}
		} catch (Exception $e) {
			file_put_contents($qrLogFile, date('Y-m-d H:i:s') . " [ERROR] " . $e->getMessage() . "\n", FILE_APPEND);
		}
	}
    // var_dump($qrImageHtml);
    // exit();
    $html2 .= "
    <tr>
    ...
    <td style=\"width:110px;vertical-align:top;\">$qrImageHtml</td>
    <td style=\"padding-left:10px;font-size:9px;vertical-align:top;\"><br><div style=\"font-size:7px;\">Чтобы произвести оплату<br>за услугу,<br>отсканируйте данный<br>ELQR</div><div style=\"font-size:8px; margin-top:10px; white-space:normal;\">Также можно оплатить через приложение Оймо Абонент, скачав его с Google Play / App Store, введя лицевой счет и нажав на \"Оплатить\"</div></td>
    </tr></table>
    </td>
    </tr>
    </td>
    ...
    <tr>
    <td style=\"border: none; text-align: right;\">Начислено</td>
    </tr>
    <tr>
    <td style=\"border: none; text-align: right;\">Карыз / Задолженность</td>
    </tr>
    <tr>
    <td style=\"border: none; text-align: right;\">Ашыкча төлөм / Переплата</td>
    </tr>
    <tr>
    <td style=\"border: none; text-align: right;\"><b>Жалпы төлөм / Итого к оплате</b></td>
    </tr>
    </td>
    <td width=\"15%\" style=\"text-align: left;\">
    <tr>
    <td width=\"15%\" style=\"text-align: right;\">" . round($total, 2) . "</td>
    </tr>
    <tr>
    <td width=\"15%\" style=\"text-align: right;\">" . round($zadolzhennost, 2) . "</td>
    </tr>
    <tr>
    <td width=\"15%\" style=\"text-align: right;\">" . round($overpayment, 2) . "</td>
    </tr>
    <tr>
    <td width=\"15%\" style=\"text-align: right;\"><b>" . round($debt, 2) . "</b></td>
    </tr>
    </td>
    </tr>\n</table>";
...
function getQrCode()
{
    ...
    $result = $adb->run_query_allrecords(
        ...
        AND vc.deleted = 0 AND vp.cf_municipal_enterprise = 8"
    );
