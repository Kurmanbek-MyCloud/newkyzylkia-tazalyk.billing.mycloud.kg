...
function getAllInvoicesData($idList)
{
    ...
    $servicesQuery = "SELECT
        ...
        COALESCE(NULLIF(inventoryproductrel.previous_reading, 0), prev_r.meter_reading, 0) as previous_reading,
        COALESCE(NULLIF(inventoryproductrel.current_reading, 0), cur_r.meter_reading, 0) as current_reading,
        ...
        LEFT JOIN vtiger_readings prev_r ON prev_r.readingsid = inventoryproductrel.prev_reading_id
        LEFT JOIN vtiger_readings cur_r ON cur_r.readingsid = inventoryproductrel.cur_reading_id
...
function getHtml($invoiceId, $qrCodeData, $allInvoicesData)
{
    ...
    $html2 = "
			...
			<tr class=\"table-head\">
				<td width=\"30%\">Коммуналдык кызмат / Ком. услуга</td>
				<td width=\"13%\" style=\"text-align: center;\">Счётчик</td>
				<td width=\"12%\" style=\"text-align: center;\">Предыдущее показание</td>
				<td width=\"12%\" style=\"text-align: center;\">Последнее показание</td>
				<td width=\"9%\" style=\"text-align: center;\">Количество</td>
				<td width=\"9%\" style=\"text-align: center;\">Тариф</td>
    ...
    foreach ($services as $row) {
        ...
        $html2 .= "
			<tr>
				...
				<td width=\"13%\" style=\"text-align: center;\">$meterNumber</td>
				<td width=\"12%\" style=\"text-align: center;\">$previousReading</td>
				<td width=\"12%\" style=\"text-align: center;\">$currentReading</td>
				<td width=\"9%\" style=\"text-align: center;\">$quantity</td>
				<td width=\"9%\" style=\"text-align: center;\">$listPrice</td>
...
<tr>
    ...
    if ($overpayment > 0) {
        ...
        <img src=\"data:image/png;base64,{$qrCodeImageBase64}\" width=\"100\" alt=\"QR-код для оплаты\" />
        </div>";
        } else {
            $html2 .= "
